...
export function applySnapshot(parts, dims, snapshot) {
  ...
  // Composter. The "composter" partId may resolve to a Group, in which
  // case we tint the first Mesh child instead. Guard the whole branch so
  // a missing piece can't crash the snapshot apply.
  ...
  if (composter && snapshot.composter?.phase) {
    ...
    let mesh = composter.material ? composter : null;
    if (!mesh && composter.traverse) {
      composter.traverse((o) => { if (!mesh && o.material) mesh = o; });
    }
    if (mesh?.material?.color?.setHex) mesh.material.color.setHex(c);
  }
